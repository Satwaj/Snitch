...
const SellerProductDetails = () => {
  ...
  const { handleGetProductById, handleAddProductVariant,deleteProductVariant } = useProduct();
  ...
  const handleDeleteVariant = async (variantId) => {
    if (!variantId) {
      alert("Cannot delete a variant without an ID. Please refresh the page.");
      return;
    }
  ...
  const handleAddNewVariant = async () => {
    ...
    if (response?.success && response.product) {
      ...
      if (response.product.variants) {
        setLocalVariants(response.product.variants);
      }
    } else {
      await fetchProductDetails();
    }
          ...
          {localVariants.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {localVariants.map((variant, idx) => (
                <div
                  key={idx}
                  className="bg-[#ffffff] flex flex-col pt-4 shadow-[0_20px_40px_rgba(27,28,26,0.02)] relative group"
                >
                  <button
                    onClick={() => handleDeleteVariant(variant._id)}
                    className="absolute top-2 right-2 text-[#ba1a1a] p-2 hover:bg-[#ffdad6] transition-colors cursor-pointer md:opacity-0 group-hover:opacity-100 focus:opacity-100 rounded-none"
